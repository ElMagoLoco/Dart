...
: mFont(0), mFPS(0.0f), mMilliSecPerFrame(0.0f), mNumTris(0), mNumVertices(0)
{
	...
	wcscpy_s(fontDesc.FaceName, sizeof(fontDesc.FaceName), _T("Times New Roman"));
...
void GfxStats::display()
{
	...
	swprintf_s(buffer, sizeof(buffer), L"Frames Per Second = %.2f\n"
		L"Milliseconds Per Frame = %.4f\n"
		L"Triangle Count = %d\n"
		L"Vertex Count = %d  [SLR]", mFPS, mMilliSecPerFrame, mNumTris, mNumVertices);
